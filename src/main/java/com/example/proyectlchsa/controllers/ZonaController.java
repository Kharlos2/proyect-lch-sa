...
public class ZonaController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Todas las zonas encontradas",
                    content = { @Content(mediaType = "application/json",
                            schema = @Schema(implementation = Zona.class)) }),
    ...
    @PostMapping("/")
    public ResponseEntity<ZonaDto> saveObject(@RequestBody Zona zona) throws Exception{
        try{
            return ResponseEntity.status(HttpStatus.CREATED).body(zonaService.saveObject(zona));
        }catch (Exception e){
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ZonaErrorDto(e.getMessage()));
        }
    }
    ...
    @PutMapping("/{idObject}")
    public ResponseEntity<ZonaDto> updateObject(@RequestBody Zona zona, @PathVariable Long idObject) throws Exception{
        try {
            return ResponseEntity.status(HttpStatus.ACCEPTED).body(zonaService.updateObject(idObject,zona));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new ZonaErrorDto(e.getMessage()));
        }
    }
